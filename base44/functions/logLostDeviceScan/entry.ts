...
/**
 * logLostDeviceScan
 * Called automatically when a lost device/asset page is opened by a finder.
 * Creates a LostItemReport (scan event — no finder details yet) and notifies
 * the owner immediately (BingooNotification + email).
 *
 * Input:
 *   { device_code, scan_source?: "nfc"|"qr"|"direct", latitude?: number, longitude?: number }
 *
 * Returns: { report_id, owner_notified }
 *
 * Privacy: this only records the scan event + (optional) approximate/approved
 * coordinates. The finder's precise location is attached later via
 * notifyLostDeviceFound only after explicit browser permission.
 */
Deno.serve(async (req) => {
  try {
    const base44 = createClientFromRequest(req);
    const body = await req.json().catch(() => ({}));
    const { device_code, scan_source, latitude, longitude } = body;
    ...
    // Only log when the device or its linked asset is actually lost.
    if (!ctx.isDeviceLost && !ctx.isAssetLost) {
      return Response.json({ skipped: true, reason: 'Device is not in lost state' });
    }
    ...
    if (ctx.ownerUserId || ctx.device.profile_id) {
      try {
        ...
        await base44.asServiceRole.entities.BingooNotification.create({
          ...
          title: `📍 Lost device scanned: ${normalizedCode}`,
          message: `Someone just scanned your lost ${label}${ctx.asset ? ` (${ctx.asset.name})` : ''} assigned to ${target}.${typeof latitude === 'number' ? ' Location shared.' : ''}`,
    ...
    if (ctx.ownerEmail) {
      try {
        ...
        const subject = `📍 Your lost ${label} (${normalizedCode}) was just scanned`;
        const html = `
          <div style="font-family:sans-serif;max-width:560px;margin:auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0">
            <div style="background:linear-gradient(135deg,#0b2149,#13284f);padding:28px;text-align:center">
              <h1 style="color:#fff;margin:0;font-size:22px">📍 Lost Device Scanned</h1>
            ...
            <div style="padding:24px">
              ...
              <p style="color:#64748b">Someone scanned your lost <strong>${label}</strong> (code <strong>${normalizedCode}</strong>)${ctx.asset ? ` linked to <strong>${ctx.asset.name}</strong>` : ''} assigned to <strong>${target}</strong>.</p>
    ...
    console.log(`[logLostDeviceScan] ${normalizedCode} report ${report.id} created. notified=${ownerNotified}`);
