  ...
  return (
    <div className="space-y-5 text-white">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-2.5 sm:gap-3">{[
        [t("mfg_paid_orders",language),orders.filter(o=>o.payment_status==="paid").length],[t("mfg_production_jobs",language),jobs.length],[t("mfg_approved_partners",language),partners.filter(p=>p.status==="approved").length],[t("mfg_qc_passed",language),qcRecords.filter(q=>q.status==="passed").length],["Shipments",shipments.length]
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">{[["queue",t("mfg_queue",language)],["partners",t("mfg_partners",language)],["qc","QC"],["shipments","Shipments"]].map(([id,label])=><Button key={id} size="sm" onClick={()=>setSection(id)} className={`min-h-[40px] flex-shrink-0 ${section===id?'bg-orange-500 hover:bg-orange-600':'bg-white/5 border border-white/15 text-white hover:bg-white/10'}`}>{label}</Button>)}</div>
      ...
      {section==='qc' && <div className="space-y-3">{qcRecords.length===0?<div className="rounded-3xl border border-white/10 bg-white/5 py-12 text-center text-white/45"><ShieldCheck className="w-10 h-10 mx-auto mb-3 opacity-30"/><p className="font-bold">No QC records yet</p><p className="text-xs mt-1">QC records appear after a production job reaches quality control.</p></div>:qcRecords.map(q=><div key={q.id} className="rounded-2xl border border-white/10 bg-white/5 p-4 flex flex-col sm:flex-row sm:items-center gap-3"><div className="min-w-0 flex-1"><p className="font-mono text-xs text-orange-300 break-all">{q.qc_number}</p><p className="font-bold mt-1">{q.device_code||'Production QC'}</p><p className="text-xs text-white/40">{q.inspected_at?new Date(q.inspected_at).toLocaleString():''}</p></div><Badge className={q.status==='passed'?'bg-emerald-500/15 text-emerald-300':'bg-white/10 text-white'}>{q.status}</Badge></div>)}</div>}

      {section==='shipments' && <div className="space-y-3">{shipments.length===0?<div className="rounded-3xl border border-white/10 bg-white/5 py-12 text-center text-white/45"><Truck className="w-10 h-10 mx-auto mb-3 opacity-30"/><p className="font-bold">No shipment records yet</p><p className="text-xs mt-1">Shipment records are created when production is ready and tracking is assigned.</p></div>:shipments.map(s=><div key={s.id} className="rounded-2xl border border-white/10 bg-white/5 p-4"><div className="flex flex-col sm:flex-row sm:items-center gap-3"><div className="min-w-0 flex-1"><p className="font-mono text-xs text-orange-300">{s.shipment_number}</p><p className="font-bold mt-1">{s.carrier||'Carrier pending'} {s.tracking_number?`· ${s.tracking_number}`:''}</p><p className="text-xs text-white/40">{s.origin_country_code||'—'} → {s.destination_country_code||'—'} · {s.service_level||'standard'}</p></div><Badge className="bg-white/10 text-white">{s.status}</Badge></div>{s.tracking_url&&<a href={s.tracking_url} target="_blank" rel="noreferrer" className="inline-flex mt-3 text-xs text-orange-300 hover:underline">Open tracking ↗</a>}</div>)}</div>}
