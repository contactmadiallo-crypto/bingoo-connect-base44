...
export default function AdminManufacturingTab() {
  ...
  const { data: shipments = [] } = useQuery({ queryKey:['admin-production-shipments'], queryFn:()=>base44.entities.Shipment.list('-created_date',500) });

  const refreshOps = async()=>{ await Promise.all(['admin-shop-orders','admin-production-jobs','admin-manufacturing-partners','admin-production-qc','admin-production-shipments'].map(queryKey=>qc.invalidateQueries({queryKey:[queryKey]}))); };
  ...
  return (
    <div className="space-y-5 text-white">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-2.5 sm:gap-3">{[
        [t("mfg_paid_orders",language),orders.filter(o=>o.payment_status==="paid").length],[t("mfg_production_jobs",language),jobs.length],[t("mfg_approved_partners",language),partners.filter(p=>p.status==="approved").length],[t("mfg_qc_passed",language),qcRecords.filter(q=>q.status==="passed").length],[language === "fr" ? "Expéditions" : "Shipments",shipments.length]
      ].map(([label,value])=><div key={label} className="rounded-2xl bg-white/5 border border-white/10 p-3.5 sm:p-4 min-w-0"><p className="text-xl sm:text-2xl font-black">{value}</p><p className="text-[11px] sm:text-xs text-white/40 break-words">{label}</p></div>)}</div>

      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">{[["queue",t("mfg_queue",language)],["partners",t("mfg_partners",language)],["qc","QC"],["shipments",language === "fr" ? "Expéditions" : "Shipments"]].map(([id,label])=><Button key={id} size="sm" onClick={()=>setSection(id)} className={`min-h-[40px] flex-shrink-0 ${section===id?'bg-orange-500 hover:bg-orange-600':'bg-white/5 border border-white/15 text-white hover:bg-white/10'}`}>{label}</Button>)}</div>
      ...
      {section==='qc' && <div className="space-y-3">{qcRecords.length===0?<div className="rounded-3xl border border-white/10 bg-white/5 py-12 text-center text-white/45"><ShieldCheck className="w-10 h-10 mx-auto mb-3 opacity-30"/><p className="font-bold">{language === "fr" ? "Aucun contrôle qualité pour le moment" : "No QC records yet"}</p><p className="text-xs mt-1">{language === "fr" ? "Les contrôles qualité apparaissent lorsqu’un lot atteint l’étape de contrôle." : "QC records appear after a production job reaches quality control."}</p></div>:qcRecords.map(q=><div key={q.id} className="rounded-2xl border border-white/10 bg-white/5 p-4 flex flex-col sm:flex-row sm:items-center gap-3"><div className="min-w-0 flex-1"><p className="font-mono text-xs text-orange-300 break-all">{q.qc_number}</p><p className="font-bold mt-1">{q.device_code || (language === 'fr' ? 'Contrôle qualité production' : 'Production QC')}</p><p className="text-xs text-white/40">{q.inspected_at?new Date(q.inspected_at).toLocaleString():''}</p></div><Badge className={q.status==='passed'?'bg-emerald-500/15 text-emerald-300':'bg-white/10 text-white'}>{q.status}</Badge></div>)}</div>}

      {section==='shipments' && <div className="space-y-3">{shipments.length===0?<div className="rounded-3xl border border-white/10 bg-white/5 py-12 text-center text-white/45"><Truck className="w-10 h-10 mx-auto mb-3 opacity-30"/><p className="font-bold">{language === "fr" ? "Aucune expédition pour le moment" : "No shipment records yet"}</p><p className="text-xs mt-1">{language === "fr" ? "Les expéditions sont créées lorsque la production est prête et qu’un suivi est attribué." : "Shipment records are created when production is ready and tracking is assigned."}</p></div>:shipments.map(s=><div key={s.id} className="rounded-2xl border border-white/10 bg-white/5 p-4"><div className="flex flex-col sm:flex-row sm:items-center gap-3"><div className="min-w-0 flex-1"><p className="font-mono text-xs text-orange-300">{s.shipment_number}</p><p className="font-bold mt-1">{s.carrier || (language === 'fr' ? 'Transporteur en attente' : 'Carrier pending')} {s.tracking_number?`· ${s.tracking_number}`:''}</p><p className="text-xs text-white/40">{s.origin_country_code||'—'} → {s.destination_country_code||'—'} · {s.service_level||'standard'}</p></div><Badge className="bg-white/10 text-white">{s.status}</Badge></div>{s.tracking_url&&<a href={s.tracking_url} target="_blank" rel="noreferrer" className="inline-flex mt-3 text-xs text-orange-300 hover:underline">{language === "fr" ? "Ouvrir le suivi" : "Open tracking"} ↗</a>}</div>)}</div>}
      ...
      {isLoading ? (
        ...
        <div className="space-y-5">
          {filtered.map(order => {
            ...
            return (
              <div key={order.id} className="rounded-3xl border border-white/10 bg-white/[0.06] p-3.5 sm:p-5 md:p-6 shadow-xl min-w-0 overflow-hidden">
                ...
                <div className="grid xl:grid-cols-3 gap-5">
                  ...
                  <div className="rounded-2xl bg-black/10 border border-white/10 p-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      ...
                      <Input type="date" className="sm:col-span-2 bg-white/5 border-white/15 text-white" value={s.estimated_delivery || ''} onChange={e => setShipping(x => ({ ...x, [order.id]: { ...s, estimated_delivery: e.target.value ? new Date(`${e.target.value}T12:00:00`).toISOString() : '' } }))} />
                      <Button className="sm:col-span-2 min-h-[44px] bg-orange-500 hover:bg-orange-600" disabled={busy === order.id} onClick={() => ship(order)}>{busy === order.id ? t("mfg_saving",language) : order.tracking_number ? t("mfg_update_shipment",language) : t("mfg_mark_shipped",language)}</Button>
                      {order.tracking_url && <a className="sm:col-span-2 text-center text-xs text-orange-300 hover:underline break-all" href={order.tracking_url} target="_blank" rel="noreferrer">{t("mfg_open_tracking",language)} ↗</a>}
                      {order.fulfillment_status === 'shipped' && <Button className="sm:col-span-2 min-h-[44px] bg-white/5 border-white/15 text-white" variant="outline" disabled={busy === order.id} onClick={() => update(order, { fulfillment_status: 'delivered' })}>{t("mfg_mark_delivered",language)}</Button>}
                    </div>
                  </div>
                </div>
              </div>
