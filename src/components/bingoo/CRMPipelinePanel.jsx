            ...
            return (
              <div key={l.id} className={`rounded-2xl border p-4 flex gap-3 items-start ${card}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className={`text-[10px] mt-1 ${dark ? "text-white/25" : "text-slate-300"}`}>{l.created_date?.slice(0, 10)}</p>
                </div>
                <div className="flex flex-col gap-1 flex-shrink-0">
                  <button onClick={() => openEdit(l)} className={`w-7 h-7 rounded-lg flex items-center justify-center transition-colors ${dark ? "hover:bg-white/10 text-white/40 hover:text-white" : "hover:bg-slate-100 text-slate-400 hover:text-slate-700"}`}><Pencil className="w-3.5 h-3.5" /></button>
                  <button onClick={() => { if (confirm("Delete lead?")) deleteMutation.mutate(l.id); }} className={`w-7 h-7 rounded-lg flex items-center justify-center transition-colors ${dark ? "hover:bg-red-500/15 text-white/30 hover:text-red-400" : "hover:bg-red-50 text-slate-300 hover:text-red-500"}`}><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              </div>
            );
          })}
