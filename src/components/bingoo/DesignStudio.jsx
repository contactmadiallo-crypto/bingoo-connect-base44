...
export default function DesignStudio({ isDark }) {
 ...
 const previewProps={productType,cardColor,accentColor,logoUrl,nameText,holderName,roleText,phone,email,website,removeBranding,finish,isDark:false,brandPattern};
 ...
 return <div className="bg-[#f7f9fc] text-slate-900 rounded-2xl overflow-hidden border border-slate-200">
   ...
   <div className="grid xl:grid-cols-[420px_minmax(460px,1fr)_300px] gap-3 p-3">
    <section className="bg-white rounded-xl border border-slate-200 p-4 space-y-5">
    ...
    <section className="space-y-3">
      <div className="bg-white rounded-xl border border-slate-200 p-4">
       ...
       <div className="min-h-[390px] rounded-xl bg-gradient-to-b from-slate-50 to-slate-100 flex items-center justify-center p-8"><ProductPreview {...previewProps} side={side}/></div>
    ...
    <aside className="bg-white rounded-xl border border-slate-200 p-4 h-fit xl:sticky xl:top-4">
      <h2 className="font-black text-[#0b2149] mb-4">Product Summary</h2><div className="flex gap-3 items-center pb-4 border-b"><div className="w-20 h-16 rounded-lg bg-slate-50 overflow-hidden flex items-center justify-center"><div className="scale-[.2]"><ProductPreview {...previewProps} side="front"/></div></div><div><p className="font-black text-sm">Custom NFC {productLabel}</p><p className="text-lg font-black mt-2">${subtotal.toFixed(2)}</p></div></div>
...
function Block({n,title,children}){return <div><h3 className="font-black text-sm mb-3"><span className="mr-1">{n}.</span>{title}</h3>{children}</div>}
