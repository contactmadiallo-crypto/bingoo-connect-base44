...
export default function DesignStudioProfessional({ isDark, profile }) {
  ...
  const productKeys = { card: 'ds_device_card', keychain: 'ds_device_key_fob', sticker: 'ds_device_sticker', bracelet: 'ds_device_bracelet', tag: 'ds_device_tag', stand: 'ds_device_table_stand' };
  ...
  const handleSaveDraft = () => {
    saveDraft({
      ...
      name: `${nameText || t("ds_untitled", language)} — ${productLabel} (Personal)`,
    });
  ...
  return (
    <div className="rounded-2xl overflow-hidden border" style={{ borderColor: BORDER }}>
      ...
      <div className="px-5 py-4 flex items-center gap-3" style={{ background: `linear-gradient(135deg, ${NAVY}, #071A3D)` }}>
        ...
        <div>
          <h2 className="text-white font-black text-base">{t("ds_personal_studio", language)}</h2>
          <p className="text-white/50 text-[10px]">{t("ds_personal_subtitle", language)}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="lg:w-72 p-5 overflow-y-auto border-b lg:border-b-0 lg:border-r" style={{ background: isDark ? '#13162a' : BG, borderColor: BORDER }}>
          {/* Product Type */}
          <div className="mb-5">
            <p className="text-xs font-black mb-2" style={{ color: labelColor }}>{t("ds_product_type", language)}</p>
            <div className="grid grid-cols-3 gap-2">
              {PRODUCT_TYPES.slice(0, 6).map((p) => (
                <button key={p.id} onClick={() => setProductType(p.id)}
                  ...
                  <span className="text-[9px] font-bold" style={{ color: productType === p.id ? ORANGE : MUTED }}>{t(productKeys[p.id] || "ds_device_card", language)}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Logo Upload */}
          <div className="mb-5">
            <p className="text-xs font-black mb-2" style={{ color: labelColor }}>{t("ds_logo_optional", language)}</p>
            <button onClick={() => fileInputRef.current?.click()}
              ...
              {logoUrl ? (
                ...
                <>
                  ...
                  <p className="text-[9px] font-bold" style={{ color: INK }}>{uploading ? t("ds_uploading", language) : t("ds_upload", language)}</p>
                </>
              )}
            </button>
          ...
          {/* Card Color */}
          <div className="mb-5">
            <p className="text-xs font-black mb-2" style={{ color: labelColor }}>{t("ds_card_color", language)}</p>
          ...
          {/* Accent Color */}
          <div className="mb-5">
            <p className="text-xs font-black mb-2" style={{ color: labelColor }}>{t("ds_accent_color", language)}</p>
            <div className="flex gap-2 flex-wrap">
              {ACCENT_COLORS.map((c) => (
          ...
          <div className="mb-5 space-y-3">
            <div>
              <p className="text-xs font-black mb-1.5" style={{ color: labelColor }}>{t("ds_name_on_card", language)}</p>
              <input value={nameText} onChange={(e) => setNameText(e.target.value)} placeholder={t("ds_your_name", language)} className={inputCls} />
            </div>
            <div>
              <p className="text-xs font-black mb-1.5" style={{ color: labelColor }}>{t("ds_role_title", language)}</p>
              <input value={roleText} onChange={(e) => setRoleText(e.target.value)} placeholder={t("ds_consultant_placeholder", language)} className={inputCls} />
            </div>
            <div>
          ...
          {/* Finish */}
          <div className="mb-5">
            <p className="text-xs font-black mb-2" style={{ color: labelColor }}>{t("ds_finish", language)}</p>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-black" style={{ color: labelColor }}>{t("ds_my_drafts", language)}</p>
              <button onClick={handleSaveDraft}
                ...
                {savedFlash ? <><Check className="w-3 h-3" /> {t("ds_saved", language)}</> : <><Save className="w-3 h-3" /> {t("ds_save", language)}</>}
              </button>
            </div>
            {drafts.length === 0 ? (
              <p className="text-[9px] text-center py-3" style={{ color: MUTED }}>{t("ds_no_drafts", language)}</p>
            ) : (
                ...
                {drafts.map(d => (
                  <div key={d.id} className="flex items-center gap-2 p-2 rounded-lg border" style={{ borderColor: BORDER, background: '#fff' }}>
                    ...
                    <button onClick={() => handleLoadDraft(d)} className="flex-1 text-left min-w-0">
                      <p className="text-[9px] font-bold truncate" style={{ color: INK }}>{d.name || t("ds_draft", language)}</p>
                    </button>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6"
          ...
          <div className="flex items-center gap-2 w-full max-w-md mb-4">
            ...
            <p className="text-sm font-black" style={{ color: isDark ? '#fff' : INK }}>{t("ds_live_preview", language)}</p>
          </div>
          <div className="mb-6 transition-transform hover:scale-105">
            ...
            <p className="text-center text-[10px] font-bold mt-3" style={{ color: MUTED }}>{t("ds_front_caps", language)}</p>
          </div>
        </div>

        {/* Right: Order Summary */}
        <div className="lg:w-56 p-5 border-t lg:border-t-0 lg:border-l" style={{ background: isDark ? '#13162a' : '#fff', borderColor: BORDER }}>
          <p className="text-xs font-black mb-4" style={{ color: labelColor }}>{t("ds_order_summary", language)}</p>
          <div className="space-y-3 mb-4">
            <div className="flex justify-between"><span className="text-[10px] font-bold" style={{ color: MUTED }}>{t("ds_product", language)}</span><span className="text-[10px] font-bold" style={{ color: labelColor }}>NFC {productLabel}</span></div>
            <div className="flex justify-between"><span className="text-[10px] font-bold" style={{ color: MUTED }}>{t("ds_finish", language)}</span><span className="text-[10px] font-bold" style={{ color: labelColor }}>{finishLabel(finish)}</span></div>
            <div className="flex justify-between items-center">
              <span className="text-[10px] font-bold" style={{ color: MUTED }}>{t("ds_quantity", language)}</span>
          ...
          <div className="rounded-xl p-3 space-y-2 mb-4" style={{ background: isDark ? '#1a1d33' : BG }}>
            <div className="flex justify-between"><span className="text-[10px] font-bold" style={{ color: MUTED }}>{t("ds_unit", language)}</span><span className="text-[10px] font-bold" style={{ color: labelColor }}>${UNIT_PRICE.toFixed(2)}</span></div>
            <div className="flex justify-between pt-2 border-t" style={{ borderColor: BORDER }}>
              <span className="text-xs font-black" style={{ color: labelColor }}>{t("ds_total", language)}</span>
          ...
          <button onClick={handlePlaceOrder}
            ...
            {ordered ? <><Check className="w-3.5 h-3.5" /> {t("ds_added", language)}</> : <><ShoppingCart className="w-3.5 h-3.5" /> {t("ds_order_card", language)}</>}
          </button>
          <p className="text-[9px] text-center mt-3" style={{ color: MUTED }}>{t("ds_shipping_checkout", language)}</p>
        </div>
      </div>
    </div>
  );
