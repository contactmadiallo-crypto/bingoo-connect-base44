...
export default function LeadCaptureSection({ profileId, color = "#0b2149", isLawFirm = false }) {
  ...
  const [metaLoading, setMetaLoading] = useState(!isLawFirm);

  const set = (k) => (e) => setForm(f => ({ ...f, [k]: e.target.value }));

  useEffect(() => {
    if (!profileId || isLawFirm) return;
    let cancelled = false;
    setMetaLoading(true);
    base44.functions.invoke("getPublicProfileCategory", { profile_id: profileId })
      .then((response) => {
        if (!cancelled) setProfileMeta(response?.data || null);
      })
      .catch(() => {
  ...
  const cta = getProfilePublicCta(profileMeta || {});
  if (metaLoading || !cta) return null;
          ...
          >
            <div className="p-5">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="font-black text-slate-900 text-base">{cta.formTitle}</h3>
                  <p className="text-slate-500 text-xs">{cta.formSubtitle}</p>
                </div>
                <button type="button" onClick={() => setOpen(false)} className="w-7 h-7 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 hover:bg-slate-200 transition-colors text-sm">✕</button>
              </div>

              <form onSubmit={handleSubmit} className="space-y-3">
                <input className="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm font-medium bg-white focus:outline-none focus:border-blue-400 transition-colors" placeholder="Full Name *" value={form.name} onChange={set("name")} />
                <input className="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm font-medium bg-white focus:outline-none focus:border-blue-400 transition-colors" placeholder="Phone Number" type="tel" value={form.phone} onChange={set("phone")} />
                <input className="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm font-medium bg-white focus:outline-none focus:border-blue-400 transition-colors" placeholder="Email Address" type="email" value={form.email} onChange={set("email")} />
                <textarea className="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm font-medium bg-white focus:outline-none focus:border-blue-400 transition-colors resize-none" placeholder="Message" rows={3} value={form.message} onChange={set("message")} />
