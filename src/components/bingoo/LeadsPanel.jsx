              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {lead.phone && (
                  <a href={`https://wa.me/${lead.phone.replace(/\D/g,"")}`} target="_blank" rel="noopener noreferrer"
                    className="flex-1 min-w-[80px] min-h-[44px] max-w-[calc(50%-0.25rem)] sm:max-w-none flex items-center justify-center py-2.5 rounded-xl text-xs font-bold text-center whitespace-nowrap bg-green-600 hover:bg-green-500 text-white transition-colors">
                    💬 WhatsApp
                  </a>
                )}
                {lead.phone && (
                  <a href={`tel:${lead.phone}`} className={`flex-1 min-w-[80px] min-h-[44px] max-w-[calc(50%-0.25rem)] sm:max-w-none flex items-center justify-center py-2.5 rounded-xl text-xs font-bold text-center whitespace-nowrap transition-colors ${isDark ? "bg-white/8 hover:bg-white/12 text-white/70" : "bg-slate-100 hover:bg-slate-200 text-slate-700"}`}>
                    📞 Call
                  </a>
                )}
                {lead.email && (
                  <a href={`mailto:${lead.email}`} className={`flex-1 min-w-[80px] min-h-[44px] max-w-[calc(50%-0.25rem)] sm:max-w-none flex items-center justify-center py-2.5 rounded-xl text-xs font-bold text-center whitespace-nowrap transition-colors ${isDark ? "bg-white/8 hover:bg-white/12 text-white/70" : "bg-slate-100 hover:bg-slate-200 text-slate-700"}`}>
                    📧 Email
                  </a>
                )}
              </div>
