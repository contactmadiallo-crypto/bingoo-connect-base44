...
export default function PortfolioPanel({ profileId, user }) {
  ...
  const create = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["portfolio", profileId] }); setForm(EMPTY); setAdding(false); toast.success(t("portfolio_item_added",language)); },
    onError: (err) => {
      const msg = err?.response?.data?.error || err?.message || t("portfolio_permission_denied",language);
      toast.error(`${t("portfolio_failed_add",language)}: ${msg}`);
    },
  });
  ...
  if (!profileId) return (
    <div className={`text-center py-12 ${mutedText}`}>{t("portfolio_profile_first",language)}</div>
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className={`text-xl font-black ${headText}`}>{t("portfolio_title",language)}</h2>
          <p className={`text-sm mt-0.5 ${mutedText}`}>{t("portfolio_subtitle",language)}</p>
        </div>
        {!adding && (
          <Button onClick={() => setAdding(true)} className="bg-blue-600 hover:bg-blue-500 text-white gap-2">
            <Plus className="w-4 h-4" /> {t("portfolio_add_item",language)}
          </Button>
        )}
      </div>
      ...
      {adding && (
        <div className="rounded-2xl p-6 space-y-4" style={{ background: formBg, border: `1px solid ${cardBorder}` }}>
          <h3 className={`font-bold ${headText}`}>{t("portfolio_new_item",language)}</h3>
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <Label className={labelClass}>{t("portfolio_title_label",language)}</Label>
              <Input className={`mt-1 ${inputClass}`} placeholder={t("portfolio_project_name",language)} value={form.title} onChange={set("title")} />
            </div>
            <div>
              <Label className={labelClass}>{t("portfolio_category",language)}</Label>
              <Input className={`mt-1 ${inputClass}`} placeholder={t("portfolio_category_ph",language)} value={form.category} onChange={set("category")} />
            </div>
          </div>
          <div>
            <Label className={labelClass}>{t("portfolio_description",language)}</Label>
            <Textarea className={`mt-1 ${inputClass}`} placeholder={t("portfolio_description_ph",language)} value={form.description} onChange={set("description")} rows={2} />
          </div>
          <div>
            <Label className={labelClass}>{t("portfolio_external_link",language)}</Label>
            <Input className={`mt-1 ${inputClass}`} placeholder="https://..." value={form.link} onChange={set("link")} />
          </div>
          <div>
            <Label className={labelClass}>{t("portfolio_cover_image",language)}</Label>
            <div className="flex items-center gap-3 mt-1">
              ...
              <label className="cursor-pointer text-xs font-semibold text-blue-500 hover:text-blue-400 bg-blue-500/10 border border-blue-500/20 px-3 py-2 rounded-lg inline-flex items-center gap-2 transition-colors">
                {uploading ? <><div className="w-3 h-3 border-2 border-blue-400 border-t-transparent rounded-full animate-spin" />{t("portfolio_uploading",language)}</> : <><Image className="w-3.5 h-3.5" />{t("portfolio_upload_image",language)}</>}
                <input type="file" accept="image/*" className="hidden" onChange={handleImageUpload} disabled={uploading} />
              </label>
            </div>
          </div>
          <div className="flex gap-3 pt-2">
            <Button variant="outline" onClick={() => { setAdding(false); setForm(EMPTY); }} className={cancelBtnClass}>{t("portfolio_cancel",language)}</Button>
            <Button disabled={!form.title || create.isPending} onClick={() => create.mutate()} className="bg-blue-600 hover:bg-blue-500 text-white">
              {create.isPending ? t("portfolio_saving",language) : t("portfolio_add_to",language)}
            </Button>
          </div>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 rounded-2xl" style={{ background: isDark ? "rgba(255,255,255,0.02)" : "#f8fafc", border: `1px dashed ${cardBorder}` }}>
          ...
          <p className={`font-semibold ${isDark ? "text-white/40" : "text-slate-500"}`}>{t("portfolio_none",language)}</p>
          <p className={`text-sm mt-1 ${isDark ? "text-white/20" : "text-slate-400"}`}>{t("portfolio_none_copy",language)}</p>
        </div>
      ) : (
          ...
          {items.map(item => (
            <div key={item.id} className="group rounded-2xl overflow-hidden flex flex-col"
              ...
              <div className="relative aspect-video w-full overflow-hidden">
                ...
                <button onClick={() => remove.mutate(item.id)} aria-label={t("portfolio_delete_aria",language)}
                ...
                {item.link && (
                  <a href={item.link} target="_blank" rel="noopener noreferrer" aria-label={t("portfolio_open_external",language)}
