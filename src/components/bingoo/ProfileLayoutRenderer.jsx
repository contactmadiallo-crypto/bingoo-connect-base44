...
/**
 * Image Hero — full-width image with minimal text overlay
 */
export function ImageHeroLayout({ profile, color, isDark, mobile, contentSections }) {
  const size = mobile ? 95 : 110;
  const radius = getAvatarRadius(profile.avatar_shape || "circle");
  ...
  return (
    <div style={{ position: "relative", borderRadius: mobile ? 0 : "24px 24px 0 0", overflow: "hidden" }}>
      <div style={{ height: mobile ? 320 : 400, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, background: `linear-gradient(180deg, transparent 40%, ${hexRgb("#000", 0.5)} 100%)` }} />
        <div style={{ position: "absolute", bottom: 20, left: 0, right: 0, display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
          <div style={{ padding: 3, background: `linear-gradient(135deg, ${color}, ${hexRgb(color, 0.6)})`, borderRadius: radius, boxShadow: `0 0 0 3px #fff` }}>
            {profile.profile_photo
              ? <img src={profile.profile_photo} alt="" style={{ width: size, height: size, borderRadius: radius, objectFit: "cover", objectPosition: "center top", display: "block" }} />
          ...
          <h1 style={{ margin: 0, fontSize: mobile ? 22 : 28, fontWeight: 900, color: "#fff", textAlign: "center" }}>{profile.display_name}</h1>
          {profile.job_title && <p style={{ margin: 0, fontSize: 13, color: "rgba(255,255,255,0.8)", fontWeight: 700 }}>{profile.job_title}</p>}
        </div>
      </div>
