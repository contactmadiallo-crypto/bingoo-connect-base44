            ...
            {step === "form" && !done && (
              <form onSubmit={handleSubmit} style={{ padding: "16px 18px 18px" }}>
                ...
                <select value={form.interested_in} onChange={e => setForm(f => ({ ...f, interested_in: e.target.value }))}
                  style={{ width: "100%", padding: "10px 12px", borderRadius: 12, background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.15)", color: "#fff", fontSize: 13, fontWeight: 600, outline: "none", marginBottom: 12, appearance: "none" }}>
                  {INTERESTS.map(i => <option key={i} value={i} style={{ background: "#0B2E6B" }}>{i}</option>)}
                </select>
