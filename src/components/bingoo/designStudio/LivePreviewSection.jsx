  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4">
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-black text-[#0b2149]"><span className="mr-1 text-[#f97316]">4.</span>Live Preview</h2>
        <div className="flex gap-1 bg-slate-100 rounded-full p-0.5">
          <button onClick={() => setPreviewView('front')}
            className={`px-3 py-1.5 rounded-full text-[11px] font-bold transition-colors ${previewView === 'front' ? 'bg-[#0b2149] text-white' : 'text-slate-500'}`}>Front</button>
          <button onClick={() => setPreviewView('back')}
            className={`px-3 py-1.5 rounded-full text-[11px] font-bold transition-colors ${previewView === 'back' ? 'bg-[#0b2149] text-white' : 'text-slate-500'}`}>Back</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2 mt-4">
        {THUMBS.map(t => (
          <button key={t.id} onClick={() => setPreviewView(t.id)}
            ...
            <p className={`text-[10px] font-bold text-center mt-1 ${previewView === t.id ? 'text-[#f97316]' : 'text-slate-600'}`}>{t.label}</p>
          </button>
        ))}
      </div>
    </div>
