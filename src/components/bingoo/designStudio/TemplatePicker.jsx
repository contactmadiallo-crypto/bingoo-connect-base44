  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-black text-sm text-[#0b2149]"><span className="mr-1 text-[#f97316]">2.</span>Select a Template</h3>
      </div>
      <div className="grid grid-cols-2 gap-2">
        {TEMPLATES.map(t => (
          <button key={t.id} onClick={() => onSelect(t)}
            ...
            <p className={`text-[10px] font-bold mt-1 text-center ${activeTemplate === t.id ? 'text-[#f97316]' : 'text-slate-700'}`}>{t.name}</p>
          </button>
        ))}
      </div>
    </div>
