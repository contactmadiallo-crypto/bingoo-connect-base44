...
function Logo({ light = false }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <div>
        <span className={`font-black text-base tracking-tight ${light ? 'text-white' : 'text-[#0b2149]'}`}>Bingoo</span>
        <span className={`text-[10px] font-bold ml-1 ${light ? 'text-white/50' : 'text-[#f97316]'}`}>CONNECT</span>
      </div>
    </div>
  );
}
