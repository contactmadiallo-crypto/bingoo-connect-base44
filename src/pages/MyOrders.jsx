...
export default function MyOrders() {
  ...
  return <div className="min-h-screen bg-slate-50">
    <div className="bg-white border-b border-slate-200"><div className="max-w-4xl mx-auto px-3 sm:px-4 py-3 sm:py-4 flex items-center gap-2 sm:gap-3 min-w-0">
      <Link to="/shop" className="text-slate-600 flex items-center gap-1"><ArrowLeft className="w-4 h-4"/> {t("shop_shop",language)}</Link>
      <h1 className="text-lg sm:text-xl font-bold text-slate-900 truncate">{t("tracking_title",language)}</h1>
    </div></div>
    <div className="max-w-4xl mx-auto px-3 sm:px-4 py-5 sm:py-8 min-w-0">
      <div className="bg-[#071b3d] text-white rounded-3xl p-4 sm:p-6 mb-6">
      ...
      <div className="space-y-6">{orders.map(order => {
        ...
        return <div key={order.id} className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
          <div className="p-4 sm:p-6 border-b flex flex-wrap justify-between gap-3"><div><p className="text-xs text-slate-400 uppercase font-bold tracking-wider">{t("order_number",language)}</p><h3 className="font-bold text-xl">{order.order_number || t('tracking_processing_number',language)}</h3><p className="text-sm text-slate-500">{new Date(order.created_date).toLocaleString()}</p></div><div className="text-right"><p className="font-bold text-xl">${Number(order.total||0).toFixed(2)}</p><span className="text-xs px-3 py-1 rounded-full bg-green-50 text-green-700 font-semibold">{t(`tracking_status_${order.payment_status}`,language) === `tracking_status_${order.payment_status}` ? order.payment_status : t(`tracking_status_${order.payment_status}`,language)}</span></div></div>
          <div className="p-4 sm:p-6">
            <div className="w-full overflow-x-auto scrollbar-hide mb-7"><div className="grid grid-cols-5 gap-2 min-w-[430px]">{STEPS.map((s,i)=><div key={s.key} className="text-center"><div className={`w-9 h-9 mx-auto rounded-full flex items-center justify-center ${i<=current?'bg-orange-500 text-white':'bg-slate-100 text-slate-400'}`}><s.Icon className="w-4 h-4"/></div><p className={`text-[11px] mt-2 ${i<=current?'font-semibold text-slate-800':'text-slate-400'}`}>{t(s.labelKey,language)}</p></div>)}</div></div>
