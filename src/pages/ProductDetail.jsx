...
function KeyFobDetailVisual() {
  return (
    <div className="relative w-[230px] h-[285px] flex items-end justify-center">
      <div
        className="absolute top-0 w-[86px] h-[86px] rounded-full border-[11px] border-slate-400"
        style={{ boxShadow: 'inset 0 3px 7px rgba(0,0,0,.25), 0 7px 18px rgba(15,23,42,.12)' }}
      />
      <div className="absolute top-[66px] w-[30px] h-[58px] rounded-full bg-slate-400" />
      <div
        className="w-[160px] h-[185px] rounded-[48%_48%_42%_42%] flex items-center justify-center"
        style={{
          background: 'linear-gradient(145deg,#070a10 0%,#121720 55%,#080b11 100%)',
          boxShadow: '0 28px 55px rgba(11,33,73,.24), inset 0 1px 0 rgba(255,255,255,.08)',
        }}
      >
        <div className="absolute inset-[1px] rounded-[48%_48%_42%_42%] opacity-[0.04]" style={{ backgroundImage: 'repeating-linear-gradient(90deg,transparent 0,transparent 2px,rgba(255,255,255,.85) 3px)' }} />
        <div className="relative flex flex-col items-center gap-2">
          <InfinityMark size={58} color={GOLD} strokeWidth={3.2} glow />
          <span className="text-[9px] font-black uppercase tracking-[0.2em] text-white/45">Bingoo Connect</span>
        </div>
      </div>
    </div>
  );
...
function ProductVisual({ product }) {
  if (product.id === 'nfc-key-fob') {
    return (
      <div className="w-full min-h-[360px] flex items-center justify-center bg-white">
        <KeyFobDetailVisual />
      </div>
    );
  }
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 md:py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10 items-start">
          <div
            ...
            style={{ background: product.id === 'nfc-key-fob' ? '#fff' : `linear-gradient(160deg, ${NAVY}, ${NAVY_DEEP})` }}
          >
          ...
          <div className="flex flex-col">
            ...
            {purchasable ? (
              ...
              <div className="flex items-baseline gap-2 mb-6">
                ...
                <span className="px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider text-white" style={{ background: '#64748b' }}>Coming Soon</span>
              </div>
            )}
              ...
              <div className="rounded-2xl border border-slate-200 p-4 mb-4 bg-white">
                ...
                <div className="flex gap-3">
                  {options.colors.map((c, i) => (
                    <button key={c.value} onClick={() => setSelectedColor(i)} className={`w-9 h-9 rounded-xl cursor-pointer border-2 transition-all ${selectedColor === i ? 'border-slate-900 scale-110 ring-2 ring-slate-200' : 'border-white hover:border-slate-300'}`} style={{ background: c.value }} title={c.name} />
                  ))}
                </div>
              ...
              <div className="rounded-2xl border border-slate-200 p-4 mb-4 bg-white">
                ...
                <div className="flex flex-wrap gap-2">
                  {options.materials.map((m, i) => (
                    <button key={m.value} onClick={() => setSelectedMaterial(i)} className={`px-3 py-2 rounded-lg text-xs font-bold transition-all ${selectedMaterial === i ? 'text-white' : 'border border-slate-200 text-slate-600 bg-white hover:border-slate-300'}`} style={selectedMaterial === i ? { background: NAVY } : {}}>{m.name}</button>
                  ))}
                </div>
              </div>
            )}
              ...
              <ul className="space-y-2">
                {product.features.map((f, i) => (
                  <li key={i} className="flex items-center gap-2.5 text-slate-700 text-sm">
                    <span className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: NAVY }}><Check className="w-3 h-3 text-white" /></span>{f}
                  </li>
                ))}
              </ul>
            </div>
            ...
            {perfectFor.length > 0 && (
              <div className="rounded-2xl border border-slate-200 p-4 mb-4 bg-white">
                ...
                <div className="flex flex-wrap gap-2">
                  {perfectFor.map(t => <span key={t} className="px-2.5 py-1 text-xs font-bold rounded-lg" style={{ background: `${NAVY}08`, color: NAVY }}>{t}</span>)}
                </div>
              </div>
            )}
            ...
            {purchasable && (
              <div className="flex items-center gap-4 mb-6">
                ...
                <div className="flex items-center rounded-xl border border-slate-200 overflow-hidden bg-white">
                  <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="px-4 py-2.5 hover:bg-slate-50 transition-colors"><Minus className="w-4 h-4 text-slate-600" /></button>
                  <span className="px-5 py-2.5 font-black text-slate-900 text-base min-w-[3rem] text-center">{quantity}</span>
                  <button onClick={() => setQuantity(quantity + 1)} className="px-4 py-2.5 hover:bg-slate-50 transition-colors"><Plus className="w-4 h-4 text-slate-600" /></button>
                </div>
            ...
            {purchasable ? (
              <div className="flex gap-3 mb-5">
                <button onClick={handleAddToCart} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border-2 text-sm font-bold transition-all" style={added ? { borderColor: '#16a34a', color: '#16a34a', background: '#f0fdf4' } : { borderColor: NAVY, color: NAVY, background: '#fff' }}>
                  {added ? <><Check className="w-4 h-4" /> Added to Cart</> : <><ShoppingCart className="w-4 h-4" /> Add to Cart</>}
                </button>
                <button onClick={handleBuyNow} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold text-white transition-all hover:opacity-90" style={{ background: `linear-gradient(135deg, ${ORANGE}, #e06800)` }}>Buy Now →</button>
              </div>
            ) : (
              <div className="mb-5">
                <div className="rounded-2xl p-4 mb-4" style={{ background: `${NAVY}08`, border: `1px solid ${NAVY}15` }}>
                  ...
                  <p className="text-xs text-slate-500 leading-relaxed">We're working on making this device available. Be the first to know when it launches.</p>
                </div>
                <button onClick={handleNotifyMe} className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold text-white transition-all hover:opacity-90" style={{ background: notified ? '#16a34a' : NAVY }}>
            ...
            {product.flow === 'asset_protection' ? (
              <>
                <div className="rounded-2xl p-4 mb-4" style={{ background: `linear-gradient(135deg, ${NAVY}, ${NAVY_DEEP})` }}>
                  <div className="flex items-center gap-2 mb-2"><Shield className="w-5 h-5 text-orange-400" /><p className="text-sm font-black text-white">Asset Protection Tag</p></div>
                  <p className="text-xs text-white/70 leading-relaxed">This tag uses Bingoo's <strong className="text-white">Asset Protection flow</strong> — not a personal profile. When someone taps your lost tag, they see your safe recovery contact info, not your personal details. NFC + QR recovery — not GPS tracking.</p>
                  <Link to="/my-nfc-devices" className="inline-flex items-center gap-1.5 mt-3 text-xs font-bold text-orange-400 hover:underline">Learn about Asset Protection →</Link>
                </div>
                <div className="rounded-2xl border border-slate-200 p-4 bg-white">
                  ...
                  {ASSET_PROTECTION_STEPS.map((s, i) => <p key={i} className="text-xs text-slate-700 font-medium mb-1.5 flex items-start gap-2"><span className="font-black flex-shrink-0" style={{ color: ORANGE }}>{i + 1}.</span> {s}</p>)}
                </div>
              </>
            ) : (
              <div className="rounded-2xl border border-slate-200 p-4 bg-white">
                ...
                {ACTIVATION_STEPS.map((s, i) => <p key={i} className="text-xs text-slate-700 font-medium mb-1.5 flex items-start gap-2"><span className="font-black flex-shrink-0" style={{ color: ORANGE }}>{i + 1}.</span> {s}</p>)}
              </div>
            )}
          </div>
        </div>
