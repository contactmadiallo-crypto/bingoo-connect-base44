...
// ── Avatar ────────────────────────────────────────────────────────────────────
function Avatar({ profile, size = 90, ring = true }) {
  ...
  return ring ? <div style={ringStyle}>{inner}</div> : inner;
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      <button onClick={() => { track("save_contact_click"); saveContact(profile); }}
        style={{ width: "100%", padding: "16px 20px", borderRadius: r, background: `linear-gradient(135deg, ${color}, ${color}cc)`, color: "#fff", fontWeight: 800, fontSize: 14, letterSpacing: "0.03em", border: "none", cursor: "pointer", boxShadow: `0 8px 24px ${hexRgb(color, 0.45)}`, transition: "all 0.2s", backdropFilter: "blur(8px)" }}
        onMouseOver={e => { e.currentTarget.style.transform = "translateY(-1px)"; e.currentTarget.style.boxShadow = `0 12px 30px ${hexRgb(color, 0.55)}`; }}
        onMouseOut={e => { e.currentTarget.style.transform = ""; e.currentTarget.style.boxShadow = `0 8px 24px ${hexRgb(color, 0.45)}`; }}>
        💾 &nbsp;Save Contact
      </button>
      <div style={{ display: "grid", gridTemplateColumns: profile.booking_enabled ? "1fr 1fr" : "1fr", gap: 10 }}>
        <button onClick={() => setInfoOpen(true)}
          style={{ padding: "14px 16px", borderRadius: r, background: dark ? "rgba(255,255,255,0.1)" : hexRgb(color, 0.07), color: dark ? "#fff" : color, fontWeight: 700, fontSize: 13, border: `1.5px solid ${dark ? "rgba(255,255,255,0.18)" : hexRgb(color, 0.25)}`, cursor: "pointer", transition: "all 0.2s", backdropFilter: "blur(8px)" }}>
          📋 &nbsp;Request Info
        </button>
        {profile.booking_enabled && (
          <button onClick={() => setBookOpen(true)}
            style={{ padding: "14px 16px", borderRadius: r, background: dark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.04)", color: dark ? "rgba(255,255,255,0.8)" : "#374151", fontWeight: 700, fontSize: 13, border: dark ? "1px solid rgba(255,255,255,0.1)" : "1px solid rgba(0,0,0,0.08)", cursor: "pointer", transition: "all 0.2s" }}>
            📅 &nbsp;Book Meeting
          </button>
        )}
      </div>
...
function LinkRow({ e, l, h, onClick, dark, profile }) {
  ...
  return (
    <a href={h} target="_blank" rel="noopener noreferrer" onClick={onClick}
      style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 18px", borderRadius: r, background: dark ? "rgba(255,255,255,0.05)" : "rgba(255,255,255,0.8)", border: dark ? "1px solid rgba(255,255,255,0.09)" : "1px solid rgba(0,0,0,0.06)", textDecoration: "none", color: dark ? "rgba(255,255,255,0.85)" : "#1e293b", fontWeight: 600, fontSize: 14, transition: "all 0.18s", backdropFilter: "blur(8px)" }}
      onMouseOver={ev => { ev.currentTarget.style.transform = "translateX(3px)"; ev.currentTarget.style.background = dark ? "rgba(255,255,255,0.09)" : hexRgb(color, 0.06); }}
      onMouseOut={ev => { ev.currentTarget.style.transform = ""; ev.currentTarget.style.background = dark ? "rgba(255,255,255,0.05)" : "rgba(255,255,255,0.8)"; }}>
...
const AmbientBg = ({ color, dark }) => (
  <>
    <div style={{ position: "fixed", top: "-10%", left: "-10%", width: "60vw", height: "60vw", maxWidth: 500, maxHeight: 500, borderRadius: "50%", background: `radial-gradient(circle, ${hexRgb(color, dark ? 0.18 : 0.12)} 0%, transparent 70%)`, pointerEvents: "none", zIndex: 0 }} />
    <div style={{ position: "fixed", bottom: "-5%", right: "-10%", width: "50vw", height: "50vw", maxWidth: 400, maxHeight: 400, borderRadius: "50%", background: `radial-gradient(circle, ${hexRgb(color, dark ? 0.12 : 0.08)} 0%, transparent 70%)`, pointerEvents: "none", zIndex: 0 }} />
    <div style={{ position: "fixed", top: "40%", right: "15%", width: "30vw", height: "30vw", maxWidth: 250, maxHeight: 250, borderRadius: "50%", background: `radial-gradient(circle, ${hexRgb(color, dark ? 0.08 : 0.06)} 0%, transparent 70%)`, pointerEvents: "none", zIndex: 0 }} />
  </>
);
            ...
            {secondary.length > 0 && (
              <div style={{ marginTop: 20 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {secondary.map(l => <LinkRow key={l.l} {...l} onClick={() => track(l.ev)} dark={dark} profile={profile} />)}
                </div>
              </div>
            )}
      ...
      <div style={{ width: "100%", maxWidth: 420, position: "relative", zIndex: 1 }}>
        <div style={{ borderRadius: 32, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.12)", backdropFilter: "blur(32px)", boxShadow: `0 40px 80px rgba(0,0,0,0.7), inset 0 1px 0 rgba(255,255,255,0.15)`, overflow: "hidden" }}>
          ...
          <div style={{ padding: "32px 26px 36px" }}>
            ...
            {profile.bio && (
              <p style={{ margin: "0 0 24px", fontSize: 13.5, lineHeight: 1.7, color: "rgba(255,255,255,0.5)", padding: "12px 14px", borderRadius: 14, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>{profile.bio}</p>
            )}
  ...
  return (
    <div style={{ minHeight: "100vh", background: `linear-gradient(160deg,${color} 0%,${color}cc 30%,#f0f4ff 75%,#f8f0ff 100%)`, display: "flex", justifyContent: "center", padding: "0 16px 60px", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ width: "100%", maxWidth: 420, position: "relative", zIndex: 1 }}>
        ...
        {/* Action card */}
        <div style={{ borderRadius: 28, background: "rgba(255,255,255,0.18)", backdropFilter: "blur(24px)", border: "1px solid rgba(255,255,255,0.3)", padding: "26px 22px", marginBottom: 14, boxShadow: "0 20px 60px rgba(0,0,0,0.15), inset 0 1px 0 rgba(255,255,255,0.3)" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {primary.length > 0 && <PrimaryCtAs links={primary} color={color} track={track} profile={profile} />}
            <Actions profile={profile} track={track} color={color} dark />
            {secondary.map(l => (
              <a key={l.l} href={l.h} target="_blank" rel="noopener noreferrer" onClick={() => track(l.ev)}
                style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 18px", borderRadius: btnRadius(profile.button_style || "pill"), background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.22)", color: "#fff", fontWeight: 600, fontSize: 14, textDecoration: "none", transition: "all 0.18s", backdropFilter: "blur(8px)" }}>
